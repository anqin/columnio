...
#include "twister/io/column_reader.h"

#include "common/system/concurrency/this_thread.h"
#include "thirdparty/glog/logging.h"

DEFINE_int32(twister_column_read_block_size, 1024 * 1024, "");
DECLARE_int64(twister_blocking_queue_time);
...
bool ColumnReader::Open(const std::string& filename,
    ...
    bool ret = LoadColumnData(m_start_buffer_ptr,
                              offset,
                              FLAGS_twister_column_read_block_size,
                              0);
    if (!ret) {
        return false;
    }
...
void ColumnReader::ReadCallback(uint32_t retry_times,
                                int64_t read_size,
                                uint32_t error_code) {
    if (read_size < 0 || error_code != ERR_FILE_OK) {
        ...
        m_queue.PushBack(m_buffer + m_load_size);
    }
...
bool ColumnReader::Close() {
    if (!m_finish_read) {
        m_finish_read = true;
        while (m_queue.TimedPopFront(&m_end_buffer_ptr,
                                     FLAGS_twister_blocking_queue_time)) {
...
bool ColumnReader::GetAvailableBuffLen(int32_t* buff_len) {
    while (!m_queue.TimedPopFront(&m_end_buffer_ptr,
                                  FLAGS_twister_blocking_queue_time)) {
        // Do nothing here, util something poped
    }
    // NULL means error occured while loading data
    if (m_end_buffer_ptr == NULL) {
        return false;
    }
