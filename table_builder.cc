...
DECLARE_uint64(twister_tablet_file_size);
DECLARE_uint64(twister_max_record_size);
...
TableBuilder::TableBuilder() {
    ...
    m_buffer.reset(new char[FLAGS_twister_tablet_file_size
                   + FLAGS_twister_max_record_size]);
}
...
bool TableBuilder::InitFileSetting(const TableOptions& options) {
    ...
    m_field_productor->SetBuffer(m_buffer.get(), FLAGS_twister_max_record_size);
    ...
    m_tablet_writer->SetBuffer(m_buffer.get() + FLAGS_twister_max_record_size,
                               FLAGS_twister_tablet_file_size);
