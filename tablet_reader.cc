...
bool TabletReader::Init(const std::string& tablet_file,
    ...
    m_file.reset(File::Open(m_tablet_file_name.c_str(),
...
bool TabletReader::LoadTabletSchema() {
    ...
    int64_t file_size = m_file->GetSize(m_tablet_file_name.c_str());
    if (file_size < length) {
    ...
    int64_t seek_ret = m_file->Seek(position, SEEK_SET);
    if (seek_ret != position) {
    ...
    seek_ret = m_file->Seek(position, SEEK_SET);
    if (seek_ret != position) {
