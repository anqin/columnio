...
#include "common/system/io/path.h"
...
#include "twister/io/column_writer.h"
...
bool TabletWriter::Dump() {
    ...
    Uuid uuid;
    uuid.Generate();
    std::string temp_file_name = ::io::path::GetDirectory(current_file_name)
        + "temp_" + uuid.ToString();

    OpenFileOptions options;
    options.cache_buf_len = 4 * 1024 * 1024;
    file.reset(File::Open(temp_file_name.c_str(),
    ...
    // 5. Rename the file.
    if (0 == File::Rename(temp_file_name.c_str(), current_file_name.c_str())) {
